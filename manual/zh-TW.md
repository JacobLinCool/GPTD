...
舊的主題式敵人沒了。一個請求現在由它的**工作負載物理**定義，而不是它的外觀。每個 archetype 帶著真實的 input/output token 數（ISL/OSL）、一個 **latency class**（IN 互動 · NR 近即時 · TO 吞吐/離線）、一個對著載入模型 `qualityBy` 判定的**各軸難度**向量、一個 **prefix share**（可快取程度），可能還有 **hazard**。
...
舊的固定 finetune 卡沒了。在 **Post-Training Studio**（在 Training Lab 裡）你衍生**你自己的 checkpoint**，無上限、可疊代——你甚至能微調一個微調過的模型。挑三件事：
...
清掉一波後，下一波的 **Incident** 會出現在橫幅上——而且和舊版不同，它的效果在建造階段與該波**真的生效**。它是建置需求，不是背景故事。21 個 incident 全取材自 2023→2026 的真實事件，主要家族：
