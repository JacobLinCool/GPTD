...
| --- | --- |
| A mechanic / serving rule / failure mode (any of the six outcomes) | `REDESIGN-BLUEPRINT.md` is the authoritative as-built design and `REFERENCE-DOSSIER.md` the real-world grounding — **cite them, do not contradict them**; `DESIGN.md` (the canonical GDD — keep it current), `manual/*.md`, in-game tutorial/tooltips if affected |
| A roster model (add/edit/correct) | **`MODEL-CATALOG.md`** first (the fact-checked source-of-truth: real developer/release/license/link/benchmarks/lineage), then `ROSTER`; the **i18n strings** if the display name needs localizing |
| A request archetype / hardware / infra node / post-training recipe / guardrail / wave / incident (stats, cost, behaviour) | `manual/*.md` tables, and the **i18n strings** for its name/description (both languages); confirm `REDESIGN-BLUEPRINT.md` still describes it correctly |
| The wave campaign (count, order, names) | `manual/*.md`, the in-game menu/HUD copy |
| Any **player-facing string** | **both** `src/i18n/en.ts` and `src/i18n/zh-TW.ts` (never hardcode — see §4), and the manuals if they quote it |
| Controls / keybindings | `manual/*.md` controls table, the in-game tutorial (Codex) |
| Tech stack / scripts / structure | `README.md` |

Docs in this repo and who they serve:

- **`REDESIGN-BLUEPRINT.md`** — the **authoritative as-built design** (§0–§10: the unified data model, the serving spine, dual-clock/SLO physics, every subsystem). This is the single source of truth for current mechanics — **cite it, never contradict it**. The CODE is the ultimate truth; verify specifics against `src/sim/**` and `src/core/types.ts`.
- **`REFERENCE-DOSSIER.md`** — the **real-world grounding** (real GPU specs, model figures, public benchmarks, serving-systems literature) the sim is calibrated against. The blueprint's `§ref` citations resolve here.
- **`DESIGN.md`** — the **canonical Game Design Document** (the readable as-built design of record). The blueprint is the deeper implementation spec; keep DESIGN.md current whenever a shipped behaviour shifts.
- **`README.md`** — developers landing on the repo. Stack, scripts, structure, links.
- **`REALISM.md`** — the realism ledger (mechanic → real-world basis, citations, known deviations), reconciled to the as-built code; `REFERENCE-DOSSIER.md` holds the underlying citations. The sim is meant to be defensible in front of LLM-infra professionals (the slogan: *"the board is the metaphor, the numbers are real"*) — challenge every new technique against its real-world tradeoff directions.
- **`manual/en.md`** — the player guide, **English (primary)**.
- **`manual/zh-TW.md`** — the player guide, **Traditional Chinese (正體中文/台灣)**. Keep it in step with the English one.
- **`AGENT-BRIDGE.md`** — the agent bridge: how a terminal agent (Claude Code / Codex) drives an already-open game tab over a localhost relay (no CDP/MCP), the HTTP API, action vocabulary, and `/state` snapshot shape. Keep in sync with `public/bridge.mjs`, `src/agent/**`, and `Game.agentAct`/`agentSnapshot`.
...
- **Add a model:** FIRST add it to **`MODEL-CATALOG.md`** (the fact-checked source-of-truth — real developer / release / license / official link / 5 benchmarks / lineage from Artificial Analysis + the official model card; mark `confidence`, use `—` and never guess). The catalog is refreshed by the `gptd-model-catalog` + `gptd-newest-models` research workflows. THEN add a `RosterEntry` to `ROSTER` with the REAL architecture (`paramsTotalB` for VRAM, `paramsActiveB` for compute, `layers` / `kvHeads` / `headDim` / `attn`) and a `bench: BenchInputs` of public benchmark %s (MMLU-Pro / LiveCodeBench / GPQA-Diamond / SWE-bench / AIME / …; omit any you cannot verify). The loader calls `qualityFromBenchmarks` to produce the 5-axis `qualityBy` — **never hand-write `qualityBy`** (that breaks the calibration invariant). Set `alignment` (an `AlignmentProfile`) for layer-1 safety, plus `real` provenance (display-only — see the `m.real` invariant). The model's proper name + `desc` live in the roster (canonical English); the `model.<id>.*` keys are the localization hook (real product names usually stay in English, so most are intentionally untranslated). Update the manuals. There are no fixed finetuned cards (`ft_agent` / `pt_giga` are gone — players mint those in the Studio).
- **Add an infra tech:** add an `InfraNodeDef` via `infra({…})` in `INFRA_NODES` with an `InfraCategory`, a flat `effects` bag that `applyInfraEffects` (`sim/research.ts`) folds into typed `s.infra`, `requires` / optional `conflicts` (e.g. chunked ⟂ disagg), `dataCost` + `compute`. Each node auto-becomes a `ResearchDef` on the infra track. Read the new switch in the `effects.ts` serving getters off `s.infra` (never `s.upgrades`). Note: GQA/MLA/MoE/reasoning are MODEL properties, NOT nodes — there is no `tech_moe` / `tech_reasoning` deploy gate and no `scale_pretrain`. Add `research.<id>.name` / `research.<id>.desc` locale strings (infra nodes render through the research panel).
...
- Keep established term choices consistent with `manual/zh-TW.md` (e.g. some technical terms stay in English: `SLA`, `GPU`, `Router`, `Cache`).
...
- **The manuals are separate files**, not generated: `manual/en.md` (English) and `manual/zh-TW.md`. Update both when content changes.
...
5. Docs updated per §0 (consistent with `REDESIGN-BLUEPRINT.md` + `REFERENCE-DOSSIER.md`; DESIGN.md header note, README, both manuals, both locales).
