...
| --- | --- |
| A mechanic / serving rule / failure mode (any of the six outcomes) | `BLUEPRINT.md` is the authoritative design spec and `REFERENCE-DOSSIER.md` the real-world grounding — **cite them, do not contradict them**; `DESIGN.md` (the canonical GDD — keep it current), `docs/manual/*.md`, in-game tutorial/tooltips if affected |
...
| A request archetype / hardware / infra node / post-training recipe / guardrail / wave / incident (stats, cost, behaviour) | `docs/manual/*.md` tables, and the **i18n strings** for its name/description (both languages); confirm `BLUEPRINT.md` still describes it correctly |
...
5. Docs updated per §0 (consistent with `BLUEPRINT.md` + `REFERENCE-DOSSIER.md`; DESIGN.md header note, README, both manuals, both locales).
