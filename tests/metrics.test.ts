...
const BIG = 'llama33_70b' // 70B dense

describe('ui/metrics — worked-example parity (§5.7 / §5.8)', () => {
  it('H100 / 70B FP8 deployed: roofline matches the §5.7 physics', () => {
    const s = createState(1)
    modernEra(s) // FP8 → bytesPerParam = 1, 70B (≈70 GB) fits an 80 GB H100
    const lo = loadout(s, H100, BIG)
    const rf = rooflineOf(s, lo)
    expect(rf.fits).toBe(true)
    // decodeTokSb1 = HBM_BW / (2 × activeB × bytes) = 3.35e12 / (2×70e9×1) ≈ 23.9
    // (the §5.7 FP16 ~12 doubles at FP8; this is the bytesPerParam halving, §5.6).
    expect(rf.decodeTokSb1).toBeGreaterThan(22)
    expect(rf.decodeTokSb1).toBeLessThan(26)
    // computeRoofTokS uses the FP8 tensor rate 1979e12 / (2×70e9) ≈ 14135 tok/s.
    expect(rf.computeRoofTokS).toBeGreaterThan(13000)
    expect(rf.computeRoofTokS).toBeLessThan(15500)
    // b=1 decode is far below the compute roof → the rack is bandwidth-bound.
    expect(rf.binding).toBe('bandwidth')
  })

  it('§5.7 FP16 raw formula: H100 / 70B decode b=1 ≈ 12 tok/s', () => {
    // The raw §5.7 physics (independent of fit/quant): 3.35e12 / (2×70e9×2) ≈ 11.96.
    const hbmBytesPerSec = 3.35e12
    const activeB = 70
    const bytesFp16 = 2
    const b1 = hbmBytesPerSec / (2 * activeB * 1e9 * bytesFp16)
    expect(b1).toBeGreaterThan(11)
    expect(b1).toBeLessThan(13)
...
describe('ui/metrics — VRAM / sparsity', () => {
  it('VRAM breakdown sums to total HBM', () => {
    ...
    // FP8 70B = ~70 GB weights on an 80 GB card.
    expect(v.weightsGb).toBeCloseTo(70, 0)
  ...
  it('dense model has sparsity ratio 1, MoE far below', () => {
    ...
    const moe = sparsityOf(loadout(s, H100, 'qwen3_30b_a3b'))
    expect(moe.isMoE).toBe(true)
    expect(moe.ratio).toBeLessThan(0.2) // 3.3 active / 30.5 total
  })
})
