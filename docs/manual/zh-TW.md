...
這份文件是給第一次打開遊戲的玩家看的。你不需要懂 AI serving，也不需要讀完整份設計藍圖。你只要記住一件事：GPTD 是一款塔防遊戲，只是你不是把敵人打死，而是把它們**服務完成**。棋盤是一座以真實數據打造的 LLM 推論資料中心：一波波使用者請求從四個入口 lane 匯入中央 Trust Core；你不是把它們打死，而是把它們服務完成——又快、又便宜、又正確、又安全——在它們漏進核心之前。
