...
**GPTD.** Base per-archetype `inputTokens`/`outputTokens` (`REQUEST_TYPES`, `src/sim/content.ts`) model the **early-era** magnitudes; the campaign then inflates them with the era. `tierContext` and `tierWork` (`src/sim/campaign.ts`) ramp **convexly** (gentle early, steep late) to ~2.9× / ~3× by the tier-12 finale, and `spawnRequest` applies `input = base × contextMul × √workMul`, `output = base × workMul` (`src/sim/spawn.ts`). A per-archetype `LENGTH_SENS` lets the **long-context / offline lanes scale fully** while **damping the tight-TTFT interactive lanes** (so they stay inside SLO instead of mass-`slo_miss`ing). Net end-of-campaign: RAG ≈ **40K→1.5K**, Summarization ≈ **60K→1.2K**, Agentic ≈ **30K→2.4K**, Reasoning ≈ **2.3K→17K** — into the real long-context band, which is what finally makes the **context window (§1.5)**, **KV budget (§1.4)**, and prefill tech (§1.6) bite in the late game. Magnitudes are validated against the autoplay depth gate (`tests/playthrough.test.ts`) so a maxed long-context fleet stays servable. The Codex Requests tab shows the `base → era` range so the displayed numbers are not read as fixed.
