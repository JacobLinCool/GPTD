...
| `agentic` | **Terminal-Bench Hard** | **SWE-bench Verified / τ²-bench** | **未飽和、最具鑑別力（天花板 ~50%）** |
| (`context` 分數) | longContext / RULER | — | — |

> **關鍵**：`agentic` 軸**唯一**來自未飽和的 Terminal-Bench Hard（SWE-bench Verified 為換算 fallback）——這是 §6.4「大模型仍值錢」與 qwen 差異化的物理依據。純規模不等於 agentic：Terminal-Bench Hard 衡量 terminal-agent 能力，連大型非終端 MoE（Qwen3-235B TB-Hard 13.6）都**真實落後**，小 MoE 亦然（校準事實，非手編）。
...
depthSurcharge  = 1 + 0.6 × baseDepth                 // fine-tune-of-fine-tune 逐步變貴
dataSizeFactor  = max(1, (base.paramsTotalB / 8)^0.45) // total params 決定資料量；MoE 不再按 active 便宜訓
sparsityFactor  = 1 + 0.12 × sqrt(max(1,totalB/activeB) − 1) // MoE resident/comm 開銷
computeCost     = recipe.costCompute × sizeFactor × effort × 1000 × POSTTRAIN_COMPUTE_SCALE × depthSurcharge × sparsityFactor
dataCost        = round(recipe.costData × effort × depthSurcharge × dataSizeFactor)
waves       = ceil(computeCost / requisitionThroughput)  // 由 research.ts 真實 aggregate TFLOPS 決定
```
> **worked example（LoRA agentic on Qwen3-30B-A3B，active 3.3B，total 30B，effort=1.0，baseDepth=0）**：`sizeFactor=(3.3/8)^0.7≈0.54`，`sparsityFactor≈1.34`；`computeCost≈1×0.54×1000×2×1.34≈1,450 FLOPS·s`。CPT 同 base：`60×0.54×1000×2×1.34≈87k`，仍 ≈60×（§2.1）。`dataCost` 同時吃 `dataSizeFactor=(30/8)^0.45≈1.81`，所以大/稀疏模型不再用 tiny active-param 的資料預算訓練。
...
roster（候選池 ~98 顆＝30 顆手寫＋68 顆 Pareto 自動挑選 `src/sim/roster.generated.ts`，再經 10% frontier-tolerance gate `withinFrontierTolerance` 收斂為 42 顆實際上場；canonical 清單與計數見 `src/sim/content.ts` MODEL_DEFS，分析見 `PARETO.md`，逐筆事實見 `MODEL-CATALOG.md`）用 §6.2 真值計算，欄位含 `paramsTotalB/paramsActiveB` + `layers/kvHeads/headDim/attn/alignment/instructFollow`。
...
| 模型 | 差異化機制 |
|---|---|
| `qwen3_30b_a3b` | 3.3B active 的小 MoE decode 最快（§5.7 active 越小越快），但**走 agentic 維度**自然分層：`difficulty`/`qualityBy[agentic]` 依 §6.3 Terminal-Bench Hard **真實落後**（校準事實，非手編）。高 wave 的 `agent` 類請求（§2.4 primaryAxis=agentic、difficulty 高、agent 線位於 TB-Hard ≈ 25）成為它過不去的牆——答得快但答不對 agentic。serving 牆（KV 與 batch budget）仍保留，但差異化主力是 agentic capability gap。 |
...
`rackWatts = tdpWattsPerGpu × gpus × utilizationFactor`（H100 700W、B200 ~1000W、GB200 1200W、NVL72 ~120kW）。冷卻硬約束：`tdpWattsPerGpu > 1000 → require cooling==='liquid'`（液冷 rack gate 在已建液冷設施，否則不可放置——§5.5 物理硬約束）。throttle 真實化：`THROTTLE_FLOOR = 0.2`；prefill 全打擊，decode 只保留輕微 memory-bound 優勢（§Splitwise，`decodeThrottle(t)=1−(1−t)×0.85`，半速 cap 時 decode 約 58%）。
